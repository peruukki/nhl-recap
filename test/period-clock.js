...
describe('periodClock', () => {
  ...
  it('should advance by second for all minutes of a period but the last one', () => {
    ...
    const clockObserver = scheduleClock(period, periodLengthInMinutes, null,
      (clock$) => clock$.take((periodLengthInMinutes - 1) * 60 + 1));
  ...
  it('should advance by a second for the last minute of any period but the 3rd one', () => {
    [1, 2, 'OT'].forEach(period => {
      ...
      const secondElements = _.range(59, -1, -1).map((second, index) =>
  ...
  it('should advance by tenth of a second for the last minute of the 3rd period', () => {
    const period = 3;
    const periodLength = 1;
    // Take only the first second to speed up and simplify the test
    const clockObserver = scheduleClock(period, periodLength, null, (clock$) => clock$.take(11));

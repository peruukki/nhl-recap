...
describe('score panel', () => {
  ...
  describe('goal delimiter', () => {
    it('should show "at" in the pre-game info', () => {
      const { meta, teams, goals } = scoresAllRegularTime.games[1];
      assertDelimiter('pre-game', { meta, teams }, goals, 'at', '');
    });

    it('should not be shown during the playback of a started game', () => {
      const { meta, teams, goals } = scoresAllRegularTime.games[1];
      assertDelimiter('playback', { meta, teams }, goals, '');
    });
    ...
    it('should show "SO" when the playback reaches shootout and the game has a shootout goal', () => {
      const { meta, teams, goals } = scoresOvertimeAndMultipleShootout.games[1];
      assertDelimiter(
        'playback',
        { meta, teams },
        goals,
...
function assertGoalCounts(
  ...
  { meta, state = 'FINAL', teams }: { meta: GameMeta; state?: GameStatus['state']; teams: Teams },
  currentGoals: Goal[],
  awayGoals: number,
  homeGoals: number,
  ...
  const teamPanels = getTeamPanels(
    Game(gameDisplay, { meta, status: { state }, teams } as GameT, currentGoals, 0),
  );
...
function assertDelimiter(
  ...
  const delimiterNode = getDelimiter(
    Game(gameDisplay, { meta, status: { state } as GameStatus, teams } as GameT, currentGoals, 0),
  );
